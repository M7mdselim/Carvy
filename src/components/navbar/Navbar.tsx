...
export function Navbar({ transparent = false }: NavbarProps) {
  ...
  return (
    <Disclosure
      as="nav"
      className={`${navbarBgClass} sticky top-0 left-0 right-0 z-50 transition-colors duration-300 navbar-container`}
    >
      {({ open }) => (
        <>
            ...
            <div className="flex h-16 md:h-20 justify-between navbar-container">
              ...
              <div className="hidden sm:ml-6 sm:flex sm:items-center">
                <div className="flex items-center gap-4">
                  ...
                  {user && (
                    <Link
                      to="/wishlist"
                      className="inline-flex items-center justify-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-indigo-700 bg-indigo-100 hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                    >
                  ...
                  {user ? <UserMenu user={user} signOut={signOut} /> : <AuthButtons />}
                </div>
              </div>
              <div className="flex items-center sm:hidden">
                <div className="flex items-center gap-2">
                  ...
                  <LanguageSwitcher className="mr-0" />
                  {user && (
          ...
          <MobileNav navigation={navigation} isCurrentPath={isCurrentPath} user={user} signOut={signOut} />
                              ...
                              <ul className="space-y-3">
                                {carModels.map((carModel) => (
                                  <li key={carModel.id}>
                                    <button
                                      onClick={() =>
                                        handleNavigation(`/products?make=${carModel.make}&model=${carModel.model}`)
                                      }
                                      className="flex items-center p-2 w-full text-left hover:bg-gray-50 rounded-md"
                                    >
                                      <div className="ml-3">
